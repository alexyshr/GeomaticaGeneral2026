# Referencia Técnica y Compendio de Comandos {#anexo-referencia}
...
### Sincronizar carpeta existente con Repositorio Vacío
Si ya tiene sus archivos en Windows y quiere subirlos a un repositorio recién creado en GitHub:

1. **Instalar Git**
2.  **Abrir Terminal en la carpeta local:**
    `git init`
    `git add .`
    `git commit -m "Carga inicial de archivos SIG"`
3.  **Vincular con la nube:**
    `git branch -M main`
...
### Vinculación del Repositorio de Contenidos {#sec-vinculacion-clase}

Una vez preparado su entorno local, el siguiente paso es vincular su espacio de trabajo con el repositorio remoto en GitHub. Existen dos protocolos principales para realizar esta tarea; la elección dependerá de si ya ha configurado su identidad digital.
...
**No sincronice estos archivos dentro de su carpeta local `ID UNAL`**. Mantener directorios separados garantiza que sus ejercicios personales y los materiales de lectura del profesor no se mezclen, evitando errores de sobreescritura y conflictos de Git al realizar actualizaciones del material.
:::
...
* **URL de la Clase**: `git@github.com:alexyshr/GeomaticaGeneral2026.git`

* **Vincular el repositorio remoto**:
    Si ya inicializó su carpeta con `git init`, ejecute el siguiente comando para establecer el origen:
```powershell
git remote add origin git@github.com:alexyshr/GeomaticaGeneral2026.git
```
...
Utilice este método si aún no ha configurado llaves SSH o prefiere una descarga rápida. A diferencia del método anterior, este suele solicitar un **Personal Access Token (PAT)** para realizar operaciones de subida (*push*).

* **URL de la Clase**: `https://github.com/alexyshr/GeomaticaGeneral2026.git`
...
```powershell
git clone [https://github.com/alexyshr/GeomaticaGeneral2026.git](https://github.com/alexyshr/GeomaticaGeneral2026.git)
```
---
...
### Configuración de Extensiones en VS Code {#sec-extensiones-vscode}
...
| **Ecosistema Python** | **Python & Debugger** | IntelliSense (Pylance) y depuración avanzada | `ms-python.python` |
| | **Jupyter & Keymap** | Soporte para Notebooks e interactividad nativa | `ms-toolsai.jupyter` |
| **Ecosistema Julia** | **Julia** | Soporte integral para ejecución y gráficos | `julialang.language-julia` |
| **Base de Datos** | **PostgreSQL** | Cliente para explorar la DB (por Chris Kolkman) | `ckolkman.vscode-postgres` |
| **Documentación** | **Quarto** | Renderizado y preview de archivos `.qmd` | `quarto.quarto` |
| **Repositorios** | **GitHub Repositories** | Manejo de archivos remotos sin clonar todo | `github.remotehub` |
...
## Comandos Rápidos y Shortcuts {#shortcuts-ref}
...
#### 2. Resolución de Conflictos (El "Fix" del Puerto)
