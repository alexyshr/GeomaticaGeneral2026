...
## (NEW - use it) Configuración de Mermaid a pdf en Docker
...
# 2. Verificación de LibGEOS (Calculamos, pero NO graficamos aún)
try
    # Usamos 'global' para que la variable sobreviva y el siguiente chunk la pueda usar
    global punto = LibGEOS.readgeom("POINT (0 0)")
    global buffer_geom = LibGEOS.buffer(punto, 1.0)
    println("✅ LibGEOS operativo: Motores geométricos verificados")
catch e
    println("❌ Error en LibGEOS: ", e)
end
...
```{r}
#| label: julia_verificacion_plot
#| eval: false
# #| echo: false
j_plot(r"-(
# Dibujamos el gráfico de forma limpia al final. 
# Al no usar 'display()', el objeto plt queda vivo y j_plot lo captura para el PDF.
plot(buffer_geom, 
     title="Validación Julia SIG: Buffer GEOS", 
     fillcolor=:blue, 
     fillalpha=0.3, 
