# Guía de Instalación. Opción A: Docker {#guia-A}
...
1.  **VSCode IDE**: Su interfaz principal para editar archivos `.qmd` y gestionar código [@vscode2026]. En el anexo de referencia técnica encontratá las principales extensiones necesarias a instalar en VSCode.
...
5.  **Verificación de Logs**: Si desea ver el progreso o verificar errores, revise el archivo `.log` generado o ejecute:

```bash
docker logs contenedor_sig_unal
```

6.  **Ingreso a JupyterLab**: Una vez el contenedor esté corriendo, abra su navegador y pegue la siguiente URL (note que usamos el puerto **8889** definido en nuestro manifiesto):

`http://127.0.0.1:8889/lab?token=geomatica2025`

7.  **Localización de Archivos y Persistencia**: En el contenedor, su carpeta de Windows se encuentra vinculada a la ruta **/home/rstudio/work**. 
    ...
    * La carpeta `imágenes` (provista para las guías Quarto) debe residir también en esta ruta para un renderizado correcto.

8.  **Acceso y Configuración en VSCode**: 
    ...
    * **Extensiones**: Una vez conectado "dentro" del contenedor, debe habilitar/instalar las extensiones de **R**, **Julia**, **Python** y **Quarto** en la sección *'Install in Container'*.
...
9.  **Accesos posteriores**: Para sesiones futuras, ya no requiere el comando `build`. Solo ejecute:
...
try:
    conn = psycopg2.connect(
        host="db-postgis", 
        dbname="sig_db_unal", 
        user="profe_unal",        # Extraído de POSTGRES_USER
        password="geomatica2025", # Extraído de POSTGRES_PASSWORD
        port="5432"               # Puerto interno de la red Docker
    )
...
# 3. Prueba de GeoPandas y Visualización
try:
    punto = Point(-74.08, 4.63) # Coordenadas aproximadas UNAL Bogotá
    buffer_geom = punto.buffer(0.01)
    ...
    gdf_buffer.plot(ax=ax, color='blue', alpha=0.3, edgecolor='black')
    gdf_punto.plot(ax=ax, color='red', markersize=50)
    
    ax.set_title("Validación: Buffer sobre UNAL Bogotá")
